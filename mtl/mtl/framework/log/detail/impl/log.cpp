...
BOOST_LOG_ATTRIBUTE_KEYWORD(log_severity, "Severity", mtl::framework::log::SeverityLevel)
...
void initLog(const std::string& directory_name)
{
    ...
    file_sink->locked_backend()->set_file_collector(sinks::file::make_collector(
                                                        keywords::target = directory_name,
                                                        keywords::max_size = 50*1024*1024,
                                                        keywords::min_free_space = 100*1024*1024
            ));

    file_sink->set_filter(log_severity >= SL_WARNING);
